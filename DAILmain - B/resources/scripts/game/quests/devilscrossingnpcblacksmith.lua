...
function gd.quests.devilsCrossingNPCBlacksmith.B18HotfixGlobalMP()
	...
	if not hotfixTriggered then
		...
		-- B32 reset for Something for Nothing and Flesh and Iron
		local kQuestSFS = 0x08932C00
		local kTaskSFS01 = 0x23908A00
		local kTaskSFS02 = 0x1C159A60
		local kQuestFAI = 0x5E4F0F00
		local kTaskFAI01 = 0xD0334000
		local kTaskFAI02 = 0xD4BD8700
		
		local questTaskStateSFS01 = player:GetQuestTaskState(kQuestSFS, kTaskSFS01)
		local questTaskStateSFS02 = player:GetQuestTaskState(kQuestSFS, kTaskSFS02)
		local questTaskStateFAI01 = player:GetQuestTaskState(kQuestFAI, kTaskFAI01)
		local questTaskStateFAI02 = player:GetQuestTaskState(kQuestFAI, kTaskFAI02)
		
		if (questTaskStateSFS01 == QuestState.Complete && questTaskStateSFS02 != QuestState.InProgress && questTaskStateSFS02 != QuestState.Complete) then
			player:GrantQuest(0x08932C00, 0x1C159A60)
			
		elseif (questTaskStateFAI01 == QuestState.Complete && questTaskStateFAI02 == QuestState.InProgress && questTaskStateFAI02 != QuestState.Complete) then
