...
--questId 0x08932C00
--taskId1 0x23908A00 kill
--taskId2 0x1C159A60 turn-in
local somethingForNothingQuest = false
local somethingForNothingReturnQuest = false
...
function gd.MPQuestControl.bestowSomethingForNothingQuestGlobalMP()
	...
	if not somethingForNothingQuest then
		...
		local questId01 = 0x08932C00
		local taskId01 = 0x1C159A60

		local questTaskState01 = player:GetQuestTaskState(questId01, taskId01)
		
		-- Begins first task of Something for Nothing, checks if player has 50 faction with Survivors and is not already on the quest
		if (player:GetFaction("SURVIVORS") >= 50 && questTaskState01 != QuestState.InProgress) then
...
function gd.MPQuestControl.bestowSomethingForNothingReturnQuestGlobalMP()

	if not somethingForNothingReturnQuest then
		somethingForNothingReturnQuest = true
		local player = Game.GetLocalPlayer()
		
		local questId01 = 0x08932C00
		local taskId01 = 0x23908A00

		local questTaskState01 = player:GetQuestTaskState(questId01, taskId01)
		
		-- Begins second task of Something for Nothing, checks if player has 50 faction with Survivors, has the All Killed token and is not already on the quest
		if (player:GetFaction("SURVIVORS") >= 50 && questTaskState01 != QuestState.InProgress && player:HasToken("DC_SOMETHINGFORNOTHING_ALLKILLED")) then
			player:GrantQuest(0x08932C00, 0x1C159A60)
...
-- Flesh and Iron
--questId 0x5E4F0F00
--taskId1 0xD0334000 kill
--taskId2 0xD4BD8700 turn-in
local fleshAndIronQuest = false
local fleshAndIronReturnQuest = false
...
function gd.MPQuestControl.bestowFleshAndIronQuestGlobalMP()
	...
	if not fleshAndIronQuest then
		...
		local questId01 = 0x5E4F0F00
		local taskId01 = 0xD4BD8700
		local questId02 = 0x08932C00
		local taskId02 = 0x1C159A60

		local questTaskState01 = player:GetQuestTaskState(questId01, taskId01)
		local questTaskState02 = player:GetQuestTaskState(questId02, taskId02)
		
		-- Begins first task of Flesh and Iron, checks if player has completed Something for Nothing and is not already on the quest
		if (questTaskState01 != QuestState.InProgress && questTaskState02 == QuestState.Complete) then
...
function gd.MPQuestControl.bestowFleshAndIronReturnQuestGlobalMP()

	if not fleshAndIronReturnQuest then
		fleshAndIronReturnQuest = true
		local player = Game.GetLocalPlayer()
		
		local questId01 = 0x5E4F0F00
		local taskId01 = 0xD0334000
		local questId02 = 0x08932C00
		local taskId02 = 0x1C159A60
		...
		-- Begins second task of Flesh and Iron, checks if player has completed Something for Nothing, has the All Killed token and is not already on the quest
		if (questTaskState01 != QuestState.InProgress && questTaskState02 == QuestState.Complete && player:HasToken("DC_FLESHANDIRON_ALLKILLED")) then
			player:GrantQuest(0x5E4F0F00, 0xD4BD8700)
