...
-- Secret Chest A11
local secretChestA11StateObjects = orderedTable()
	secretChestA11StateObjects["SECRETCHEST_A11"	] = { state = ChestState.Looted, dbr = "records/items/lootchests/b01_chestmetal01.dbr"}
	secretChestA11StateObjects[""					 	] = { state = ChestState.Default, dbr = "records/items/lootchests/secretchests/secretchest_a11.dbr"}

local secretChestA11Spawned = false

function gd.dungeonChests.secretChests.secretA11OnAddToWorld(objectId)

	if Server then
		local player = Game.GetLocalPlayer()
		if (not secretChestA11Spawned && player:GetLevel() > 2) then
			secretChestA11Spawned = true
			
			local userdata = {}
			TokenStateBasedObjectSwap(objectId, userdata, secretChestA11StateObjects)
...
gd.dungeonChests.secretChests.secretA11OnDestroy = ClearObjectUserdata

function gd.dungeonChests.secretChests.secretA11ChestFound()
...
-- Secret Chest A12
local secretChestA12StateObjects = orderedTable()
	secretChestA12StateObjects["SECRETCHEST_A12"	] = { state = ChestState.Looted, dbr = "records/items/lootchests/b01_chestmetal01.dbr"}
	secretChestA12StateObjects[""					 	] = { state = ChestState.Default, dbr = "records/items/lootchests/secretchests/secretchest_a12.dbr"}

local secretChestA12Spawned = false

function gd.dungeonChests.secretChests.secretA12OnAddToWorld(objectId)

	if Server then
		local player = Game.GetLocalPlayer()
		if (not secretChestA12Spawned && player:GetLevel() > 2) then
			secretChestA12Spawned = true
			
			local userdata = {}
			TokenStateBasedObjectSwap(objectId, userdata, secretChestA12StateObjects)
...
gd.dungeonChests.secretChests.secretA12OnDestroy = ClearObjectUserdata

function gd.dungeonChests.secretChests.secretA12ChestFound()
